...
void salvarModificacaoCorte() {
	...
	if (std::wstring(aux1) == std::wstring(idd) && (std::wstring(aux1).size() + std::wstring(idd).size() > 1)) {	//Verificando se os IDS sao iguais.
		...
		if (w_status == L"Editando") {
			std::string comm = "update corte set nome = '" + nome + "', valor = " + valor + " where id = " + wstringToString(idd) + ";";
			...
			std::string comm = "insert into corte (nome, valor, id) values ('" + nome + "', " + valor + ", " + wstringToString(idd) + ");";
