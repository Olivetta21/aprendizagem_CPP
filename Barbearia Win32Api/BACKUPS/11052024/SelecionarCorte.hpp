...
void preencherLVCortes(HWND& lv, wchar_t* additSearch = nullptr) {
	...
	std::vector<corteInfo> ci;

	sqlite3_stmt* stmt = nullptr;


	std::string pesquisa = "select id, UPPER(nome), valor from corte order by UPPER(nome);";
	if (additSearch != nullptr && std::wcslen(additSearch) > 0) {
		pesquisa = "select id, UPPER(nome), valor from corte where nome like '%" + wstringToString(additSearch) + "%' order by UPPER(nome);";
	}
