...
public:
	corteFromDB(int corteID) {
		std::string command = "select id, UPPER(nome), valor from corte where id = " + std::to_string(corteID) + ";";
		sqlite3_stmt* stmt = nullptr;
...
public:
	...
	void setVlr(long double _vlr) {
		vlr = truncTwo(_vlr);
	}
	void setDesc(long double _desc) {
		desc = truncTwo(_desc);
	}
...
class tempAddCorte {
	corteInfo ci;
	HWND *valor, *desconto, *quantidade, *total, *chamadora;
	std::wstring last_valor, last_desconto, last_quantidade;
public:
	tempAddCorte(HWND &vlr, HWND& desc, HWND& qnt, HWND& tot, HWND& cham)
		: valor(&vlr), desconto(&desc), quantidade(&qnt), total(&tot), chamadora(&cham) {
	...
	void verifyAtts() {	//Essa funcao eh acessada incessantemente pela chamadora.
		...
		wchar_t desc[256];
		GetWindowText(*desconto, desc, 256);
		wchar_t vlr[256];
		GetWindowText(*valor, vlr, 256);

		//Se valor, desconto ou total for modificado, recalcular o total.
		if (std::wstring(qnt) != last_quantidade || std::wstring(desc) != last_desconto || std::wstring(vlr) != last_valor) {
			last_quantidade = qnt;
			last_desconto = desc;
			last_valor = vlr;

			ci.setVlr(wcToD(vlr));
			ci.setDesc(wcToD(desc));
			ci.qnt = wcToD(qnt);

			SetWindowText(*total, _PickMoney(ci.getVlr() * ci.qnt - ci.getDesc()).c_str());
		}
	}
